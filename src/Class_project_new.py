...
def classproject():
    ...
    def Mobile_barplot_by_year(select_year):
        ...
        for i,(year,color) in enumerate(zip(yearsm,year_colors)):
            ...
            axm.barh(Genrename,widths,left=starts,label=year,color=color)
            xcenters=starts+widths /2
            axm.text(1, 0.4, select_year, transform=axm.transAxes, color='#777777', size=52, ha='right', weight=800)
        
        axm.legend(ncol=len(yearsm), bbox_to_anchor=(0, 1),loc='lower left', fontsize='medium',handletextpad=0.1,handlelength=0.8,columnspacing=1)
        return axm
    ...
    def Steam_barplot_by_year(select_year):
        ...
        year_colors=plt.get_cmap('tab20')(list(next(itcolor) for _ in range(Countvalue.shape[1])))
        #year_colors=plt.get_cmap('Blues')(np.linspace(0.15,0.85,Countvalue.shape[1]))
        ...
        for i,(year,color) in enumerate(zip(years,year_colors)):
            ...
            ax.barh(Genrename,widths,left=starts,label=year,color=color)
            xcenters=starts+widths /2
            ax.text(1, 0.4, select_year, transform=ax.transAxes, color='#777777', size=52, ha='right', weight=800)
        
        ax.legend(ncol=10, bbox_to_anchor=(0, 1),loc='lower left', fontsize='medium',handletextpad=0.1,handlelength=0.8,columnspacing=0.6)
        return ax
    
    #Steam_barplot_by_year(2015)
